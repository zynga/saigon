...
<div id="nrpeplugin" style="border-width:2px;width:97%;left:5;top:45;position:absolute" class="admin_box_blue divCacGroup admin_border_black">
<?php
} else {
?>
<div id="nrpeplugin" style="border-width:2px;width:97%;left:5;top:5;position:absolute" class="admin_box_blue divCacGroup admin_border_black">
<?php
}
?>
<form method="POST" action="action.php" name="nrpe_plugin_action_write" enctype="multipart/form-data">
...
?>
        <th colspan="2">Add Supplemental NRPE Plugin to <?php echo $deployment?></th>
<?php
} else if ($action == 'modify_plugin') {
?>
        <th colspan="2">Modify Supplemental NRPE Plugin <?php echo $pluginName?> in <?php echo $deployment?></th>
<?php
}
?>
    </thead>
...
<?php
if (preg_match('/,/', GFS_ACCESS)) {
    $matches = preg_split('/,/', GFS_ACCESS);
    if (in_array($viewData->deployment, $matches)) {
?>
                <option value="<?php echo GFS_PLUGLOC?>" <?php echo is_selected($pluginLocation, GFS_PLUGLOC)?>><?php echo GFS_PLUGLOC?></option>
<?php
    }
} elseif (($viewData->deployment == GFS_ACCESS) || (GFS_ACCESS == '*')) {
?>
                <option value="<?php echo GFS_PLUGLOC?>" <?php echo is_selected($pluginLocation, GFS_PLUGLOC)?>><?php echo GFS_PLUGLOC?></option>
<?php
}
