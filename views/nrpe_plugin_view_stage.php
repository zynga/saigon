...
if ($action == 'delete') {
?>
        <th colspan="2">Delete NRPE Plugin <?php echo $pluginName?> in <?php echo $deployment?></th>
<?php
} else if ($action == 'copy_to_write') {
?>
        <th colspan="2">Copy NRPE Plugin <?php echo $pluginName?> from <?php echo $deployment?> to another deployment</th>
<?php
} else {
?>
        <th colspan="2">View NRPE Plugin <?php echo $pluginName?> in <?php echo $deployment?></th>
<?php
}
?>
    </thead>
...
<?php
    if ((isset($viewData->ccs)) && ($viewData->ccs === true)) {
?>
            <input type="hidden" value="<?php echo $viewData->deployment?>" id="todeployment" name="todeployment" />
...
<div id="subcancelbuttons">
    <a href="action.php?controller=nrpeplugin&action=delete_plugin&deployment=<?php echo $deployment?>&plugin=<?php echo $pluginName?>" class="deployBtn" title="Delete">Delete</a>
    <a href="action.php?controller=nrpeplugin&action=stage&deployment=<?php echo $deployment?>" class="deployBtn" title="Cancel">Cancel</a>
</div>
<div class="divCacGroup"><!-- 5 Pixel Spacer --></div>
<?php
} else {
...
<div class="divCacGroup"><!-- 5 Pixel Spacer --></div>
<div id="subcancelbuttons">
    <a href="action.php?controller=nrpeplugin&action=stage&deployment=<?php echo $deployment?>" class="deployBtn" title="NRPE Plugins">NRPE Plugins</a>
</div>
<div class="divCacGroup"><!-- 5 Pixel Spacer --></div>
<?php
}
