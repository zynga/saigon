...
//
class PCMDBDeploymentsException extends Exception {}

class PCMDBDeployments implements HostAPI
{

    public function getList()
    {
        $urlappend = 'api/cmdb/getdeployments';
    ...
    public function getSearchResults($input)
    {
        ...
        $urlappend = 'api/cmdb/getnagioshosts/deployment/' . $param;
        $results = $this->getdata($urlappend);
        return $results;
    }
    ...
    private function getdata($urlappend)
    {
        ...
        if ($errno) {
            throw new PCMDBDeploymentsException($errno." ".$errstr);
        }
